...
class TestExtraTree(unittest.TestCase):
    ...
    def test_main(self):
        ...
        with patch.object(self.extra_tree, 'train_model', return_value=MagicMock()) as mock_train_model, \
             patch.object(self.extra_tree, 'test_model', return_value=self.y_pred) as mock_test_model:

            # Mock metrics.metrics return values
            mock_acc_score = 0.95
            mock_cr_report = "classification report"
            mock_cnf_matrix = np.array([[1, 0], [0, 1]])
            self.extra_tree.metrics.metrics.return_value = (mock_acc_score, mock_cr_report, mock_cnf_matrix)

            result = self.extra_tree.main(self.x_train, self.x_test, self.y_train, self.y_test)
            ...
            self.extra_tree.metrics.metrics.assert_called_once_with(y_true=self.y_test, y_predicted=self.y_pred)
