...
class TestReadData(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_logger_instance = (
            self.mock_logger_class.return_value.get.return_value
        ) = MagicMock()
    ...
    def test_prepare_data(self, mock_read_csv):
        ...
        dummy_df_chunk1 = pd.DataFrame(
            {"message": ["email1 subject\nbody1", "email2 subject\nbody2"]}
        )
        dummy_df_chunk2 = pd.DataFrame(
            {"message": ["email3 subject\nbody3", "email4 subject\nbody4"]}
        )
        ...
        def mock_get_sub_message(df):
            ...
            df["message_body"] = df["message"].apply(
                lambda x: x.split("\\n")[1] if "\\n" in x else ""
            )
            return df
        ...
        self.mock_process_data_instance.get_sub_message.side_effect = (
            mock_get_sub_message
        )
        ...
        self.assertEqual(
            self.mock_process_data_instance.get_sub_message.call_count, 2
        )  # Called for each chunk
        self.assertEqual(
            self.mock_process_data_instance.clean_df.call_count, 2
        )  # Called for each chunk
        ...
        self.assertIn(
            "message_body", result_df.columns
        )  # Added by mock_get_sub_message

    @patch("Core.ReadData.read_csv")
    def test_prepare_tagged_data(self, mock_read_csv):
        # Dummy CSV content for actions.csv
        csv_content = (
            "action one, some other data\n"
        ...
        mock_read_csv.assert_called_once_with(
            "/fake/tagged/path/actions.csv", header=None
        )
        self.assertIsInstance(result_df, pd.DataFrame)
        ...
        pd.testing.assert_series_equal(
            result_df["labels"],
            pd.Series([True, True, True], name="labels"),
            check_dtype=False,
        )
