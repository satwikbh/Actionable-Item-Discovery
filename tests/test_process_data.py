...
class TestProcessData(unittest.TestCase):
    ...
    def test_get_regex_pattern(self):
        expected_pattern = r'(?:Message-ID: )([\s\S]*)(?:\n)' \
                           r'(?:Date: )([\s\S]*)(?:\n)' \
                           r'(?:From: )(.*)' \
                           r'(?:(?:(?:\n)(?=(?:To:) )(?:To: )([\s\S]*)(?:\n))|(?:(?:\n)(?!(?:To: ))))' \
                           r'(?:Subject: )([\s\S]*)(?:\n)' \
    ...
    def test_clean_text_no_match(self):
        subject = "Meeting"
        message = "Hello"
        self.assertEqual(self.process_data.clean_text(subject, message), [subject, message])
    ...
    def test_get_sub_message(self, mock_from_pandas):
        ...
        mock_df.message = mock_df.message.astype(str) # Ensure it's string type for .str accessor
    ...
    def test_clean_df(self, MockLogic, mock_from_pandas, mock_load):
        ...
        mock_logic_instance.logic_heuristic_model.return_value = True # Simulate a positive heuristic result
        ...
        with patch.object(pd.Series, 'apply', side_effect=[
            pd.Series([['test', 'subject'], ['re:', 'reply']]),  # For subject cleaning
            pd.Series([['test', 'message', 'body'], ['forwarded', 'email']])  # For message cleaning
        ]):
            # Mock the final apply call that uses logic_heuristic_model
            with patch.object(pd.DataFrame, 'apply', side_effect=lambda func: pd.Series([
                mock_logic_instance.logic_heuristic_model(
                    nlp=mock_nlp,
                    subject=row['subject'], # Pass the actual subject from the row
                    message=row['message']  # Pass the actual message from the row
                ) if row['labels'] else False
